...
public class KenKenPaModel {
    ...
    private void validateLandAnnotation() {
        ...
        for (Map.Entry<String, Element> entry : mLandMap.entrySet()) {
            ...
            if (!mDefaultState.equals(entry.getKey()) && !hasToState(entry.getKey())) {
                KenKenPaHelper.logParsingError(mProcessingEnv, entry.getValue(), Land.class, new IllegalArgumentException(
                        String.format("No state %s set Hop(to)", entry.getKey())));
            }
        }
    }
    ...
    private boolean hasNoParameters(Element element, Class<? extends Annotation> aClass) {
        ...
        if (parameters.size() > 0) {
            KenKenPaHelper.logParsingError(mProcessingEnv, element, aClass, new IllegalArgumentException(String.format(
                    "%s has too many parameter(s): %s", element.getSimpleName(),
                    parameters.size())));
            return false;
        }
        return true;
...
//            KenKenPaHelper.logParsingError(mProcessingEnv, element, TakeOff.class, new IllegalArgumentException(String.format(
